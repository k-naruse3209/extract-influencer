...
| データ取得 | Instagram API with Instagram Login 経由でプロフィール・投稿データを自動取得 |
...
- Meta Developer アカウント（Instagram API with Instagram Login 連携用）
...
| `INSTAGRAM_CLIENT_ID` | Meta App ID | `884895251041997` |
| `INSTAGRAM_CLIENT_SECRET` | Meta App Secret | Developer Console で取得 |
| `INSTAGRAM_REDIRECT_URI` | OAuth コールバック URL | `http://localhost:3001/api/v1/auth/instagram/callback` |
| `INSTAGRAM_API_VERSION` | 利用する Instagram API バージョン | `v21.0` |
| `INSTAGRAM_OAUTH_SCOPES` | Instagram Login で要求するスコープ | `instagram_business_basic,instagram_business_manage_insights` |
...
4. Instagram Login 画面が表示されるので、連携対象の Instagram Business / Creator アカウントでログイン
5. アクセス許可を承認
...
1. **データ取得** ボタンをクリック
2. Instagram API with Instagram Login 経由でプロフィールデータが取得されます（バックグラウンドで処理）
...
> **取得制約:** 連携済みアカウントと異なる対象プロフィールでは、API の仕様上メディア詳細やインサイトの一部が `UNAVAILABLE` として保存されます。連携済み本人アカウントでは詳細データを取得できます。
...
| 外部 API | Instagram API with Instagram Login |
