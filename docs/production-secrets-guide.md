...
| `INSTAGRAM_CLIENT_ID` | Instagram API with Instagram Login の Meta App ID | 必須 | `1234567890` | 高 |
| `INSTAGRAM_CLIENT_SECRET` | Instagram API with Instagram Login の Meta App Secret | 必須 | `abcdef1234567890...` | 高 |
| `INSTAGRAM_REDIRECT_URI` | Instagram OAuth コールバック URL | 必須 | `https://app.example.com/api/v1/auth/instagram/callback` | 低 |
| `INSTAGRAM_API_VERSION` | 使用する Instagram API バージョン | 必須 | `v21.0` | 低 |
| `INSTAGRAM_OAUTH_SCOPES` | Instagram Login で要求するスコープ一覧（カンマ区切り） | 必須 | `instagram_business_basic,instagram_business_manage_insights` | 低 |
...
#### INSTAGRAM_API_VERSION / INSTAGRAM_OAUTH_SCOPES

```
名前: INSTAGRAM_API_VERSION
値: v21.0

名前: INSTAGRAM_OAUTH_SCOPES
値: instagram_business_basic,instagram_business_manage_insights
```

`INSTAGRAM_API_VERSION` は provider 実装が利用する Instagram API バージョンです。`INSTAGRAM_OAUTH_SCOPES` は Instagram Login で要求するスコープをカンマ区切りで指定します。
...
env:
  ...
  INSTAGRAM_API_VERSION: ${{ secrets.INSTAGRAM_API_VERSION }}
  INSTAGRAM_OAUTH_SCOPES: ${{ secrets.INSTAGRAM_OAUTH_SCOPES }}
...
- [ ] `INSTAGRAM_API_VERSION` が GitHub Secrets に設定されている
- [ ] `INSTAGRAM_OAUTH_SCOPES` が GitHub Secrets に設定されている
...
- [ ] `INSTAGRAM_API_VERSION` がサポート対象の Meta API バージョンになっている
- [ ] `INSTAGRAM_OAUTH_SCOPES` が `instagram_business_basic,instagram_business_manage_insights` と一致している
